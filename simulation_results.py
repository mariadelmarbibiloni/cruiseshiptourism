...
if __name__ == "__main__":
    ...
    if not af_weight_eval:
        results = pd.read_csv(
                f'test_sim/data_6500_1it_{decision_method}/palma_poi_summary_{ntourists}_{time}_{aggregation_function}_{decision_method}_noise_sigma_{noise_sigma}_it-{niterations}_agg-{ct_agglomeration}.csv',
                    header=0,
                )
    else:
        results = pd.read_csv(
            f'test_sim/data_6500_1it_{decision_method}/palma_poi_summary_{ntourists}_{time}_{aggregation_function}_{decision_method}_noise_sigma_{noise_sigma}_afw_{af_weight}_it-{niterations}_agg-{ct_agglomeration}.csv',
            header=0,
        )
    ...
    if af_weight_eval:
        plt.savefig(f'test_sim/data_6500_1it_{decision_method}/summary_plots/summary_heat_map_{ntourists}_{time}_{aggregation_function}_{decision_method}_noise_sigma_{noise_sigma}_afw_{af_weight}_it-{niterations}_agg-{ct_agglomeration}.png', dpi=199)
    else:
        plt.savefig(f'test_sim/data_6500_1it_{decision_method}/summary_plots/summary_heat_map_{ntourists}_{time}_{aggregation_function}_{decision_method}_noise_sigma_{noise_sigma}_it-{niterations}_agg-{ct_agglomeration}.png', dpi=199)
